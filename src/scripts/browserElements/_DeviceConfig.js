...
function iPhoneModelDetect() {
  ...
  if (width/height === 375/667 && dpr === 2) {
    return { iPhone, model: '8/SE' }
  }
  if (width/height === 414/736 && dpr === 3) {
    return { iPhone, model: '8Plus' }
  }
  if (width/height === 375/812 && dpr === 3) {
    return { iPhone, model: 'X/XS/11Pro/12Mini' }
  }
  if (width/height === 414/896) {
    return { iPhone, model: dpr === 2 ? 'XR/11' : 'XSMax/11ProMax' }
  }

  if (width/height === 390/844 && dpr === 3) {
    return { iPhone, model: '12/12Pro' }
  }
  if (width/height === 428/926 && dpr === 3) {
    return { iPhone, model: '12ProMax' }
  }
  else {
    return {
      iPhone,
